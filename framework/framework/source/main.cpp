...
void timeout(int value)
{
	...
	if (atb_feed_back&(ATB_REPREFACTORIZE | ATB_CHANGE_TIME_STEP))
	{
...
void grab_screen(void)
{
	uchar* bitmapData = new uchar[3 * g_screen_width * g_screen_height];
...
void grab_screen(char* filename)
{
	...
	for (int i = 0; i < g_screen_height; i++)
	{
		glReadPixels(0, i, g_screen_width, 1, GL_RGB, GL_UNSIGNED_BYTE, bitmapData + (g_screen_width * 3 * ((g_screen_height - 1) - i)));
	}
